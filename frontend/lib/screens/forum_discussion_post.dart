...
// Data Service untuk mengelola data chat (memakai model yang sudah dipindahkan)
// NOTE: Ini masih dummy data service. Untuk aplikasi nyata, Anda akan menggunakan Cubit/Bloc
// dan mengambil data dari API backend.
class ForumPostDataService {
  static final ForumPostDataService _instance = ForumPostDataService._internal();
  factory ForumPostDataService() => _instance;
  ForumPostDataService._internal();
  ...
  final List<ChatMessage> _messages = [
    ChatMessage(
      id: "msg_001",
      chatSessionId: "session_001",
      senderType: "ai_bot", // Perhatikan ini, di forum harusnya 'user'
      messageContent: "Aku banget ini! Kadang sampe telat kerja karena susah bangun. Coba deh sleep hygiene-nya diperbaiki dulu, kayak matiin gadget 1 jam sebelum tidur. Terus juga coba atur jadwal tidur yang konsisten setiap hari.",
      senderId: "user_001",
      senderName: "Sarah",
      timestamp: DateTime.now().subtract(const Duration(minutes: 45)),
...
class _ForumDiscussionPostScreenState extends State<ForumDiscussionPostScreen> {
  ...
  final ForumPostDataService _dataService = ForumPostDataService(); // Menggunakan ForumPostDataService
  ...
  void initState() {
    ...
    _messageController.addListener(() {
      setState(() {
        _currentMessage = _messageController.text;
        _isTyping = _currentMessage.isNotEmpty || _pendingAttachments.isNotEmpty; // isTyping juga dari attachments
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.transparent, // Tetap transparan jika ada background Stack di bawahnya
      resizeToAvoidBottomInset: true,
      body: SafeArea(
        child: SizedBox(
          width: screenWidth,
  ...
  Widget _buildMainContent(BuildContext context, double screenWidth, double screenHeight) {
    return Stack(
      children: [
        _buildBlurBackground(),
        _buildScrollableContent(context),
        _buildBottomMessageArea(context), // Pass context
        if (_showEmojiPicker) _buildSimpleEmojiPicker(context), // Pass context
        if (_pendingAttachments.isNotEmpty && !_showEmojiPicker) _buildUploadingIndicator(context), // Pass context
        _buildArrowButton(context),
      ],
    );
  }
  ...
  Widget _buildBlurBackground() {
    return Positioned.fill(
      child: Image.asset(
        'assets/images/blur_background.png',
        width: context.screenWidth, // Gunakan context.screenWidth
        height: context.screenHeight, // Gunakan context.screenHeight
        fit: BoxFit.cover,
      ),
    );
  }
  ...
  Widget _buildScrollableContent(BuildContext context) {
    return Positioned(
      top: context.scaleHeight(111),
      left: context.scaleWidth(41),
      right: context.scaleWidth(41),
      bottom: _showEmojiPicker ? context.scaleHeight(250) : context.scaleHeight(74),
      child: SingleChildScrollView(
        controller: _scrollController,
        physics: const ClampingScrollPhysics(),
        child: Column(
          children: [
            _buildMainDiscussionCard(context), // Pass context
            SizedBox(height: context.scaleHeight(10)),
            ..._buildDiscussionMessages(context), // Pass context
  ...
  Widget _buildMainDiscussionCard(BuildContext context) {
    final discussion = _dataService.currentDiscussion;

    return Container(
      width: context.scaleWidth(348), // Use scaled width
      decoration: const BoxDecoration(
        image: DecorationImage(
          image: AssetImage('assets/images/kotak_hijau_tosca.png'),
          fit: BoxFit.fill,
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(height: context.scaleHeight(19)),
          Padding(
            padding: EdgeInsets.only(left: context.scaleWidth(15), right: context.scaleWidth(16)),
            child: _buildTitleSection(discussion.title, context), // Pass context
          ),
          SizedBox(height: context.scaleHeight(29)),
          Padding(
            padding: EdgeInsets.only(left: context.scaleWidth(15), right: context.scaleWidth(16)),
            child: _buildQuestionSection(discussion.content, context), // Pass context
          ),
          SizedBox(height: context.scaleHeight(8)),
          Padding(
            padding: EdgeInsets.only(left: context.scaleWidth(15), right: context.scaleWidth(16), bottom: context.scaleHeight(14)),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text(
                  'oleh ${discussion.authorName}',
                  style: GoogleFonts.fredoka(
                    fontSize: context.scaleWidth(9),
                    fontWeight: FontWeight.w300,
                    color: AppColor.navyText.withOpacity(0.6), // Use AppColor
                  ),
                ),
                Text(
                  TimeFormatter.formatTime(discussion.createdAt),
  ...
  Widget _buildTitleSection(String title, BuildContext context) {
    return Container(
      ...
      padding: EdgeInsets.all(context.scaleWidth(10)), // Use scaled padding
      child: Text(
        title,
        style: GoogleFonts.fredoka(
  ...
  Widget _buildQuestionSection(String content, BuildContext context) {
    return Container(
      width: context.scaleWidth(317), // Use scaled width
      decoration: const BoxDecoration(
        image: DecorationImage(
          image: AssetImage('assets/images/question_box.png'),
          fit: BoxFit.fill,
      ...
      child: Stack(
        children: [
          Container(
            width: double.infinity,
            padding: EdgeInsets.only(
              left: context.scaleWidth(10),
              right: context.scaleWidth(40),
  ...
  List<Widget> _buildDiscussionMessages(BuildContext context) {
    return _messages.asMap().entries.map((entry) {
      final index = entry.key;
      final message = entry.value;
      ...
      return Container(
        margin: EdgeInsets.only(
          bottom: index == _messages.length - 1 ? 0 : context.scaleHeight(20),
        ),
        child: Column(
          crossAxisAlignment: message.isOwner
              ? CrossAxisAlignment.end
          ...
          children: [
            if (!message.isOwner || _shouldShowSenderInfo(index))
              Padding(
                padding: EdgeInsets.only(
                  left: message.isOwner ? context.scaleWidth(50) : 0,
                  right: message.isOwner ? 0 : context.scaleWidth(50),
                  ...
                  children: [
                    if (!message.isOwner) ...[
                      Text(
                        message.senderName,
                        style: GoogleFonts.fredoka(
                          fontSize: context.scaleWidth(10),
                          fontWeight: FontWeight.w500,
                          color: AppColor.navyText.withOpacity(0.7), // Use AppColor
                        ),
                      ),
                    ...
                    Text(
                      ...
                      style: GoogleFonts.fredoka(
                        fontSize: context.scaleWidth(9),
                        fontWeight: FontWeight.w300,
                        color: AppColor.navyText.withOpacity(0.5), // Use AppColor
                      ),
                    ),
                    if (message.isOwner) ...[
                      SizedBox(width: context.scaleWidth(8)),
            ...
            _buildDiscussionMessage(message, context), // Pass context
            if (message.attachments.isNotEmpty)
              _buildAttachmentsDisplay(message.attachments, message.isOwner, context), // Pass context
          ],
        ),
      );
    }).toList();
  ...
  Widget _buildDiscussionMessage(ChatMessage message, BuildContext context) {
    return Align(
      alignment: message.isOwner ? Alignment.centerRight : Alignment.centerLeft,
      child: Container(
        constraints: BoxConstraints(
          maxWidth: context.scaleWidth(250),
        ...
        padding: EdgeInsets.all(context.scaleWidth(25)), // Use scaled padding
        child: Text(
          message.messageContent,
          style: GoogleFonts.fredoka(
  ...
  Widget _buildAttachmentsDisplay(List<AttachmentFile> attachments, bool isOwner, BuildContext context) {
    return Align(
      alignment: isOwner ? Alignment.centerRight : Alignment.centerLeft,
      child: Container(
        margin: EdgeInsets.only(top: context.scaleHeight(8)),
        constraints: BoxConstraints(maxWidth: context.scaleWidth(250)),
        child: Column(
          crossAxisAlignment: isOwner ? CrossAxisAlignment.end : CrossAxisAlignment.start,
          children: attachments.map((attachment) => _buildAttachmentItem(attachment, context)).toList(), // Pass context
        ),
      ),
    );
  }

  Widget _buildAttachmentItem(AttachmentFile attachment, BuildContext context) {
    return Container(
      margin: EdgeInsets.only(bottom: context.scaleHeight(4)),
      padding: EdgeInsets.symmetric(horizontal: context.scaleWidth(12), vertical: context.scaleHeight(8)),
      decoration: BoxDecoration(
        color: AppColor.navyText.withOpacity(0.1), // Use AppColor
        borderRadius: BorderRadius.circular(context.scaleWidth(8)),
        border: Border.all(
          color: AppColor.navyText.withOpacity(0.2), // Use AppColor
          width: 1,
        ),
      ),
      child: Row(
  ...
  Widget _buildBottomMessageArea(BuildContext context) {
    double dynamicHeight = _calculateMessageBoxHeight(context);

    return Positioned(
      bottom: context.scaleHeight(16),
      left: context.scaleWidth(10),
      right: context.scaleWidth(10),
      child: GestureDetector(
        onTap: () {
          FocusScope.of(context).requestFocus(_messageFocusNode);
        },
        child: Stack(
          ...
          children: [
            SizedBox(
              ...
              child: Image.asset(
                'assets/images/message_box.png',
                width: context.scaleWidth(300), // Ini mungkin redundant jika parent SizedBox sudah punya width
                height: dynamicHeight,
                fit: BoxFit.fill,
              ),
            ),
            if (_pendingAttachments.isNotEmpty)
              Positioned(
                left: context.scaleWidth(140),
                right: context.scaleWidth(70),
                bottom: dynamicHeight - context.scaleHeight(25),
                child: _buildPendingAttachmentsPreview(context), // Pass context
              ),
            Positioned(
              left: context.scaleWidth(140),
              right: context.scaleWidth(70),
              bottom: context.scaleHeight(10),
              top: _pendingAttachments.isNotEmpty ? context.scaleHeight(35) : context.scaleHeight(0), // Adjusted top to 0 for text field only
              child: TextField(
                controller: _messageController,
                focusNode: _messageFocusNode,
                maxLines: null,
            ...
            Positioned(
              ...
              child: GestureDetector(
                onTap: () => _toggleEmojiPicker(), // No need to pass context here
                child: Image.asset(
                  'assets/images/happy_emoji.png',
            ...
            Positioned(
              left: context.scaleWidth(87),
              bottom: context.scaleHeight(9),
              child: GestureDetector(
                onTap: () => _showAttachmentDialog(), // No need to pass context here
                child: Image.asset(
                  'assets/images/paper_clip.png',
  ...
  double _calculateMessageBoxHeight(BuildContext calcContext) {
    double baseHeight = calcContext.scaleHeight(50);

    if (!_isTyping && !_messageFocusNode.hasFocus && _pendingAttachments.isEmpty) {
      return baseHeight;
    }

    double attachmentHeight = _pendingAttachments.isNotEmpty ? calcContext.scaleHeight(35) : 0;

    final textPainter = TextPainter(
      text: TextSpan(
        text: _currentMessage.isEmpty ? 'Ketik pesan...' : _currentMessage,
        style: GoogleFonts.fredoka(
          fontSize: calcContext.scaleWidth(12),
          fontWeight: FontWeight.w400,
        ),
      ),
      maxLines: null,
    ...
    double availableWidth = calcContext.scaleWidth(417 - 140 - 70);
    if (availableWidth <= 0) availableWidth = calcContext.scaleWidth(100);

    textPainter.layout(maxWidth: availableWidth);

    double textHeight = textPainter.height;
    // Tambahan padding vertikal untuk teks, sesuaikan dengan desain Anda
    if (_currentMessage.isNotEmpty) {
      textHeight += calcContext.scaleHeight(20);
    }

    double calculatedHeight = baseHeight + attachmentHeight + textHeight;

    return calculatedHeight.clamp(baseHeight, calcContext.scaleHeight(200));
  }

  Widget _buildSimpleEmojiPicker(BuildContext context) {
    return Positioned(
          ...
          children: [
            Row(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () => setState(() => _showEmojiPicker = false),
                  child: Icon(Icons.close, size: context.scaleWidth(24)),
                ),
              ],
            ),
            SizedBox(height: context.scaleHeight(8)),
  ...
  void _showAttachmentDialog() {
    FocusScope.of(context).unfocus(); // Unfocus text field when dialog opens
